...
//echo $_GET['userName']; die();
	...
	include('classes/employeeCrud.php');
	 ...
	 $recs = $viewJobObj->viewJobs($jobId);
		...
		echo "Login in or Signup as a new user to access this page";
		header("refresh:5; url= user_login.php");die;
	}
	 	$id 		=   $recs[0]['id'];
		$title 		=	$recs[0]['title'];
			...
			<div class="row">
					<div id="job-manager-job-dashboard" class="col-md-12">
						<div>
							...
							<button name="apply" onclick="applyAction(<?=$id?>)">Apply</button>
							<button name="bookmark" onclick="bookmarkAction(<?=$id?>)">Bookmark</button>
						</div>
						</div>
					</div>
			</div>
