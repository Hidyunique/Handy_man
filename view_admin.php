...
//echo $_SESSION['userType']; die();
	...
	include('classes/adminCrud.php');
	 ...
	 $viewAdminObj = new adminCrudClass();
	 $recs = $viewAdminObj->viewAdmin();

	}


	 	//$jobId = (isset($_GET['id']) != "") ? isset($_GET['id'] : "";
...
<div class="container-fluid" style="font-size: 15px;"> 
					...
					<div id="job-manager-job-dashboard" class="col-md-12">
						<div>
							...
							<table class="job-manager-jobs table table-bordered table-striped">
								<thead>
									<tr>
										<th class="job_title">Company 		 </th>
										<th class="date">	  Username 		 </th>
										<th class="status">	  Status 		 </th>
										<th class="expires">  phone 		 </th>
										<th class="filled">   Date Registered</th>
									</tr>
								</thead>
								<tbody>
									<?php 
									 //    echo"data<pre>";
										...
										// echo"</pre>";
										 foreach($recs as $row){
											...
											echo "<tr>
													<td class='job_title'>
														<a href='#' class='job_title_link'>{$row['company_name']}</a>
														<ul class='job-dashboard-actions'>
															<li><a href='view_admin_details.php?id={$row['id']}' class='job-dashboard-action-edit'>View</a></li>
											<li><a onclick=\"deleteConfirm({$row['id']})\" href='#' >Delete</a></li>
														</ul>
													</td>
													<td class='date'>{$row['username']}</</td>
													<td class='status'><div class=''><a onclick ='approveAdmin({$row['id']})' href='#'> {$row['status']}</a></div></td>
													<td class='expires'>{$row['phone']}</td>
													<td class='filled'><a onclick='filledStatus({$row['id']})' href='#'> {$row['date']} </a></td>
													</tr>";
	...
	/*unction deleteConfirm(id){
			var id = id;
			alert(id);
			if(confirm("Are you sure you want to delete this record")){

				header("location:delete_job.php?id=");
			}
